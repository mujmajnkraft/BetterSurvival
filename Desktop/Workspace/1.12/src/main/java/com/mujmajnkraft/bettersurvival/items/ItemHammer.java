...
public class ItemHammer extends ItemSword implements ICustomWeapon{
	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
	{
		...
		float d = Math.min(this.getMaterial().getDamageVsEntity(), 9) + 1;
	    ...
	    for (BlockPos position : positions)
	    {
	    	...
	    	if (pos.distanceSq(position.getX(), position.getY(), position.getZ()) >= 2*(d-2))
	    	{
	    		if (worldIn instanceof WorldServer)
                {
                    WorldServer worldserver = (WorldServer)worldIn;
                    worldserver.spawnParticle(EnumParticleTypes.CRIT, position.getX() + 0.5f, position.getY()+1.0D, position.getZ() + 0.5f, 1, 0.0D, 0.0D, 0.0D, 0.0D, new int[0]);
                }
	    	}
	    	if (result != null)
	    	{
